...
- **配置漂移巡检**：节点上的 config-monitor Agent（DaemonSet）定期对比实际配置与 CR 声明，发现漂移自动标记并触发重新同步
- **变更检测**：基于内容 Hash 对比，仅当配置模板实际变更时才触发节点同步，避免重复执行
- **Webhook 校验**：创建/更新 CR 时自动注入默认值并校验字段合法性，配合 cert-manager 管理 TLS 证书

## 架构设计

```mermaid
graph TB
    subgraph 控制面 [控制面 - Operator]
        A[NodeGroup CR] --> B[NodeGroup Controller]
        B --> C[生成 Ansible Playbook YAML]
        B --> D[为每个 IP 创建 NodeInstance CR]
        D --> E[NodeInstance Controller]
        E --> F[通过 SSH 执行 Ansible Playbook]
    end

    subgraph 节点面 [节点面 - DaemonSet]
        H[config-monitor Agent] --> I[定期巡检节点实际配置]
        I --> J{与 CR 声明对比}
        J -->|有漂移| K[Patch NodeInstance 状态为 PendingSync]
        J -->|无漂移| L[跳过]
    end

    F --> G[目标节点: 包安装/配置文件/sysctl/服务重启]
    K --> E
```
